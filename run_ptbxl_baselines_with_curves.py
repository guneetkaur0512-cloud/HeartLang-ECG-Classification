...
class ResNet1D(nn.Module):
    ...
    def forward(self, x):
        return self.fc(self.blocks(self.stem(x.transpose(1, 2))).squeeze(-1))


class GRU(nn.Module):
    def __init__(self):
        super().__init__()
        self.gru = nn.GRU(96, 128, batch_first=True, bidirectional=True)
        self.fc = nn.Linear(256, 5)
...
class Linear(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(nn.Flatten(), nn.Linear(256 * 96, 5))
...
def main():
    ...
    ap.add_argument("--curve_out", default="results/baseline_training_curves.csv")
    ...
    Xtr = np.load(d / "train_data.npy").astype("float32")
    ytr = np.load(d / "train_labels.npy").astype("float32")

    Xv = np.load(d / "val_data.npy").astype("float32")
    yv = np.load(d / "val_labels.npy").astype("float32")

    Xt = np.load(d / "test_data.npy").astype("float32")
    yt = np.load(d / "test_labels.npy").astype("float32")
    ...
    for ep in range(1, args.epochs + 1):
        ...
        print(
            f"model={args.model} seed={args.seed} epoch={ep}: "
            f"train_loss={train_loss:.6f} "
            f"val_auc={val['auc']:.6f} "
            f"val_f1={val['f1']:.6f} "
    ...
    print("TEST", test)
    print("saved final results to", args.out)
    print("saved training curves to", args.curve_out)
